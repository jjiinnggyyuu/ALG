...
from lp_utils import apply_low_pass_filter, get_lp_strength
...
class LTXImageToVideoPipeline(DiffusionPipeline, FromSingleFileMixin, LTXVideoLoraLoaderMixin):
    ...
    def __call__(
        ...
        lp_filter_type: str = "down_up",
        lp_filter_in_latent: bool = True,
        lp_resize_factor: float = 0.25,
        lp_blur_sigma: float = 1.0,
        lp_blur_kernel_size: float = 0.05,
        lp_strength_schedule_type: str = "interval",
        schedule_interval_start_time: float = 0.0,
        schedule_interval_end_time: float = 0.1,
        schedule_linear_start_weight: float = 1.0,
        schedule_linear_end_weight: float = 0.0,
        schedule_linear_end_time: float = 0.5,
        schedule_exp_decay_rate: float = 5.0,
        schedule_blur_kernel_size: bool = False,
    ):
        r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # ===== ALG 시작 =====
                if use_low_pass_guidance:
                    lp_strength = get_lp_strength(
                        step_index=i,
                    ...
                    if lp_strength > 0:
                        ...
                        # 첫 프레임에 LPF 적용
                        first_frame = latents_unpacked[:, :, :1]  # [B, C, 1, H, W]
                        first_frame_filtered = apply_low_pass_filter(
                            first_frame,
                            filter_type=lp_filter_type,
                            blur_sigma=lp_blur_sigma,
                            blur_kernel_size=lp_blur_kernel_size,
                            resize_factor=lp_resize_factor * lp_strength,
                        )
                        # 필터된 첫 프레임으로 교체
                        latents_unpacked[:, :, :1] = first_frame_filtered
                        # 다시 팩
                        latents = self._pack_latents(
                            latents_unpacked,
                            self.transformer_spatial_patch_size,
